...
class CLICommand(object):
        ...
        if hasattr(args, 'page_size') and args.page_size > 0:
            ...
            if objects is not None and len(objects) > 0: 
                while True:
                    ...
                    if tmp_objects is None or len(objects) == 0:
                        break
                    objects += tmp_objects
    ...
    def _check_arguments(cls, args):
        ...
        args.username = args.username if args.username else os.environ.get("TDL_USERNAME", None)
        args.password = args.password if args.password else os.environ.get("TDL_PASSWORD", None)
        args.api = args.api if args.api else os.environ.get("TDL_API_URL", None)
        args.version = args.version if args.version else os.environ.get("TDL_API_VERSION", None)
        args.enterprise = args.enterprise if args.enterprise else os.environ.get("TDL_ENTERPRISE", None)
        args.json = True if os.environ.get("TDL_JSON_OUTPUT") == "True" else args.json

        if args.username is None or len(args.username) == 0:
            Printer.raise_error("Please provide a username using option --username or TDL_USERNAME environment variable")

        if args.password is None or len(args.password) == 0:
            Printer.raise_error("Please provide a password using option --password or TDL_PASSWORD environment variable")

        if args.api is None or len(args.api) == 0:
            Printer.raise_error("Please provide an API URL using option --api or TDL_API_URL environment variable")

        if args.enterprise is None or len(args.enterprise) == 0:
            Printer.raise_error("Please provide an enterprise using option --enterprise or TDL_ENTERPRISE environment variable")
