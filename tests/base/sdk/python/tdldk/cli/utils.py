...
class SDKInspector(object):
    ...
    def _get_package(self):
        ...
        if self._sdk is None:
            try:
                self._sdk = importlib.import_module("tdldk.%s" % self._version)
                self._logger = importlib.import_module("tdldk.utils").set_log_level
                self._ignored_resources = [self._sdk.SDKInfo.root_object_class().rest_name]

                # Printer.info("Imported tdldk.%s." % self._version)
            except ImportError:
                self._sdk = importlib.import_module("tdldk")
    ...
    def get_user_session(self, args):
        ...
        session = self._sdk.GATDLSession(username=args.username, password=args.password, enterprise=args.enterprise, api_url=args.api)
        try:
            session.start()
            ...
            if status_code == 401:
                ...
                Printer.raise_error("Cannot access %s [HTTP %s]. Current tdldk version tried to connect to the Server API %s" % (args.api, status_code, args.version))
